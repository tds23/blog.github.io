...
version="2.2.4", jQuery=function(selector,context){return new jQuery.fn.init(selector,context);},
...
}else if(context.nodeName.toLowerCase()!=="object"){ if((nid=context.getAttribute("id"))){nid=nid.replace(rescape,"\\$&");}else{context.setAttribute("id",(nid=expando));} 
...
if(!div.querySelectorAll("a#"+expando+"+*").length){rbuggyQSA.push(".#.+[+~]");}});assert(function(div){
 var input=document.createElement("input");input.setAttribute("type","hidden");div.appendChild(input).setAttribute("name","D");
...
lock:function(){locked=queue=[];if(!memory){list=memory="";}
...
return chainable?elems: bulk?fn.call(elems):len?fn(elems[0],key):emptyGet;};var acceptData=function(owner){



 return owner.nodeType===1||owner.nodeType===9||!(+owner.nodeType);};function Data(){this.expando=jQuery.expando+Data.uid++;}
Data.uid=1;Data.prototype={register:function(owner,initial){var value=initial||{};
 if(owner.nodeType){owner[this.expando]=value;

}else{Object.defineProperty(owner,this.expando,{value:value,writable:true,configurable:true});}
return owner[this.expando];},cache:function(owner){if(!acceptData(owner)){return{};} 
var value=owner[this.expando]; if(!value){value={};if(acceptData(owner)){
 if(owner.nodeType){owner[this.expando]=value;

}else{Object.defineProperty(owner,this.expando,{value:value,configurable:true});}}}
return value;},set:function(owner,data,value){var prop,cache=this.cache(owner); if(typeof data==="string"){cache[data]=value;}else{ for(prop in data){cache[prop]=data[prop];}}
return cache;},get:function(owner,key){return key===undefined?this.cache(owner):owner[this.expando]&&owner[this.expando][key];},access:function(owner,key,value){var stored;
...
return value!==undefined?value:key;},remove:function(owner,key){var i,name,camel,cache=owner[this.expando];if(cache===undefined){return;}
if(key===undefined){this.register(owner);}else{ if(jQuery.isArray(key)){
...
var rbrace=/^(?:\{[\w\W]*\}|\[[\w\W]*\])$/,rmultiDash=/[A-Z]/g;function dataAttr(elem,key,data){var name;
 if(data===undefined&&elem.nodeType===1){name="data-"+key.replace(rmultiDash,"-$&").toLowerCase();data=elem.getAttribute(name);if(typeof data==="string"){try{data=data==="true"?true:data==="false"?false:data==="null"?null:+data+""===data?+data:rbrace.test(data)?jQuery.parseJSON(data):data;}catch(e){} 
...
return access(this,function(value){var data,camelKey;



if(elem&&value===undefined){
 data=dataUser.get(elem,key)|| 
dataUser.get(elem,key.replace(rmultiDash,"-$&").toLowerCase());if(data!==undefined){return data;}
camelKey=jQuery.camelCase(key);
 data=dataUser.get(elem,camelKey);if(data!==undefined){return data;}
 
data=dataAttr(elem,camelKey,undefined);if(data!==undefined){return data;}
return;}
camelKey=jQuery.camelCase(key);this.each(function(){
...
return adjusted;}
var rcheckableType=(/^(?:checkbox|radio)$/i);var rtagName=(/<([\w:-]+)/);var rscriptType=(/^$|\/(?:java|ecma)script/i);var wrapMap={ option:[1,"<select multiple='multiple'>","</select>"],
...
input.setAttribute("type","radio");input.setAttribute("checked","checked");input.setAttribute("name","t");div.appendChild(input);
 support.checkClone=div.cloneNode(true).cloneNode(true).lastChild.checked; div.innerHTML="<textarea>x</textarea>";support.noCloneChecked=!!div.cloneNode(true).lastChild.defaultValue;})();var
...
if(delegateCount&&cur.nodeType&&(event.type!=="click"||isNaN(event.button)||event.button<1)){for(;cur!==this;cur=cur.parentNode||this){
if(cur.nodeType===1&&(cur.disabled!==true||event.type!=="click")){matches=[];for(i=0;i<delegateCount;i++){handleObj=handlers[i];sel=handleObj.selector+" ";if(matches[sel]===undefined){matches[sel]=handleObj.needsContext?jQuery(sel,this).index(cur)>-1:jQuery.find(sel,this,null,[cur]).length;}
...
return this.pushStack(ret);};});var iframe,elemdisplay={
HTML:"block",BODY:"block"};function actualDisplay(name,doc){var elem=jQuery(doc.createElement(name)).appendTo(doc.body),display=jQuery.css(elem[0],"display"); elem.detach();return display;}
...
div.style.backgroundClip="content-box";div.cloneNode(true).style.backgroundClip="";support.clearCloneStyle=div.style.backgroundClip==="content-box";container.style.cssText="border:0;width:8px;height:0;top:0;left:-9999px;"+"padding:0;margin-top:1px;position:absolute";container.appendChild(div);
function computeStyleTests(){div.style.cssText=
...
return boxSizingReliableVal;},pixelMarginRight:function(){

if(boxSizingReliableVal==null){computeStyleTests();}
return pixelMarginRightVal;},reliableMarginLeft:function(){ if(boxSizingReliableVal==null){computeStyleTests();}
return reliableMarginLeftVal;},reliableMarginRight:function(){
...
if(elem.nodeType===1&&("height"in props||"width"in props)){
 ...
 display=jQuery.css(elem,"display");checkDisplay=display==="none"?dataPriv.get(elem,"olddisplay")||defaultDisplay(elem.nodeName):display;if(checkDisplay==="inline"&&jQuery.css(elem,"float")==="none"){style.display="inline-block";}}
...
fxNow=undefined;};jQuery.fx.timer=function(timer){jQuery.timers.push(timer);if(timer()){jQuery.fx.start();}else{jQuery.timers.pop();}};jQuery.fx.interval=13;jQuery.fx.start=function(){if(!timerId){timerId=window.setInterval(jQuery.fx.tick,jQuery.fx.interval);}};jQuery.fx.stop=function(){window.clearInterval(timerId);timerId=null;};jQuery.fx.speeds={slow:600,fast:200, _default:400};jQuery.fn.delay=function(time,type){time=jQuery.fx?jQuery.fx.speeds[time]||time:time;type=type||"fx";return this.queue(type,function(next,hooks){var timeout=window.setTimeout(next,time);hooks.stop=function(){window.clearTimeout(timeout);};});};(function(){var input=document.createElement("input"),select=document.createElement("select"),opt=select.appendChild(document.createElement("option"));input.type="checkbox";support.checkOn=input.value!==""; support.optSelected=opt.selected;
 select.disabled=true;support.optDisabled=!opt.disabled; input=document.createElement("input");input.value="t";input.type="radio";support.radioValue=input.value==="t";})();var boolHook,attrHandle=jQuery.expr.attrHandle;jQuery.fn.extend({attr:function(name,value){return access(this,jQuery.attr,name,value,arguments.length>1);},removeAttr:function(name){return this.each(function(){jQuery.removeAttr(this,name);});}});jQuery.extend({attr:function(elem,name,value){var ret,hooks,nType=elem.nodeType; if(nType===3||nType===8||nType===2){return;} 
...
var location=window.location;var nonce=jQuery.now();var rquery=(/\?/);
jQuery.parseJSON=function(data){return JSON.parse(data+"");};jQuery.parseXML=function(data){var xml;if(!data||typeof data!=="string"){return null;} 
...
$(window).load(function(){var titlePackage=$('.single-page h1').text();$('#package_title_input').prop('value',titlePackage);});$(document).ready(function(){setTimeout(function(){$('.preload').addClass('active');},100);setTimeout(function(){$('section.main, .main .menu, .main .smi').addClass('active');$('.main').trigger('activeClassAdded');},1200);setTimeout(function(){$('section.page, .menu, .smi').addClass('active');},200);setTimeout(function(){$('.preload').css('opacity','0.2');},2000);$('.left-click').on('click',function(){$('.box-screen-right').toggleClass('active');$('.mail').addClass('hide');$('section.main.active').addClass('form');});$('.right-click').on('click',function(){$('.box-screen-left').toggleClass('active');$('.mail').addClass('hide');$('section.main.active').addClass('form');});$('.box-screen-right .back, .box-screen-left .back').on('click',function(){$(this).parent().removeClass('active');$('.mail').removeClass('hide');$('section.main.active').removeClass('form');});$('.step_back_shop,.step_back_site').on('click',function(){$(this).parent().parent().parent().parent().parent().parent().removeClass('active');$('.mail').removeClass('hide');$('section.main.active').removeClass('form');});$('.main').trigger('activeClassAdded');$('.main').on('activeClassAdded',function(){setTimeout(function(){var videobg=document.getElementById("videobg");videobg.play();},1400);});$('.about-us-slider').slick({infinite:true,slidesToShow:4,slidesToScroll:1,arrows:true,nextArrow:'<span class="slick-next"><img src="/img/arrow-down-black.png"></span>',prevArrow:'<span class="slick-prev"><img src="/img/arrow-down-black.png"></span>',responsive:[{breakpoint:1200,settings:{slidesToShow:3,slidesToScroll:1,autoplay:false,}},{breakpoint:992,settings:{slidesToShow:2,autoplay:false,}},{breakpoint:768,settings:{slidesToShow:1,autoplay:false,}}]});$('.portfolio-slider').slick({infinite:true,slidesToShow:1,slidesToScroll:1,arrows:true,fade:true,nextArrow:'<span class="slider-arrow slider-next"><img src="/img/arrow-right.png"></span>',prevArrow:'<span class="slider-arrow slider-prev"><img src="/img/arrow-left.png"></span>'});$('.screen-slider').slick({infinite:true,slidesToShow:1,slidesToScroll:1,arrows:true,fade:true,lazyLoad:'progressive',nextArrow:'<span class="slider-arrow slider-next"><img src="/img/arrow-right.png"></span>',prevArrow:'<span class="slider-arrow slider-prev"><img src="/img/arrow-left.png"></span>'}).on('beforeChange',function(event,slick,currentSlide,nextSlide){$('.slick-btn.stop').click();$('.screen-slider .slick-active img').animate({top:'0',},100);});function sliderImgAnim(){var
imgHeight=$('.screen-slider .slick-active span img').height(),imgBoxHeight=$('.screen-slider .slick-active .img-box').height(),imgHeightAdd=imgHeight-imgBoxHeight,animDur=imgHeight*3;$('.play').hide();$('.stop').show();$('.screen-slider .slick-active span img').animate({top:'-'+imgHeightAdd+'px',},animDur);resetAnimation=setTimeout(function(){$('.screen-slider .slick-active span img').animate({top:'0',},500);$('.stop').hide();$('.play').show();},animDur);}
setTimeout(sliderImgAnim,1000);$('.reload').on('click',function(){$('.stop').click();$('.screen-slider .slick-active span img').animate({top:'0',},100);clearTimeout(resetAnimation);setTimeout(sliderImgAnim,200);});$('.stop').on('click',function(){$('.screen-slider .slick-active span img').stop();$('.play').show();$('.slick-media-btn.play').css('opacity','0.6');$('.stop').hide();clearTimeout(resetAnimation);});$('.play').on('click',function(){sliderImgAnim();$('.stop').show();});$('.screen-slider .img-box').hover(function(){$('.slick-media-btn').css('opacity','0.6');});$('.screen-slider .img-box').mouseleave(function(){$('.slick-media-btn').css('opacity','0');$('.slick-media-btn.play').css('opacity','0.6');});var imgBoxHeight=function(){var
a=$('.screen-slider .img-box .monitor-bg').height(),b=$('.screen-slider .img-box span').height(),c=a+b;$('.screen-slider .img-box').height(c);var
d=$('.screen-slider .img-box').width(),e=d/2-50;$('.slick-media-btn').css('left',e+'px');}
imgBoxHeight();$(window).on('resize',imgBoxHeight);$('.popup-open').on('click',function(){var rel=$(this).attr('rel');$('.popup-'+rel).addClass('active');return false;});$('.popup-close').on('click',function(){$('.popup').removeClass('active');return false;});$('.navbar-button').on('click',function(){$('.mobile').toggle('ease');$(this).parent().toggleClass('dark');});$(window).load(function(){$('.order-services').parent().addClass('order-services');if($(window).width()>768){$('.menu-main-menu-container').removeClass('mobile');}else{$('.menu-main-menu-container').addClass('mobile');}});$(window).resize(function(){if($(window).width()>768){$('.menu-main-menu-container').removeClass('mobile');$('.menu-main-menu-container').removeAttr("style");$('nav').removeClass('dark');}else{$('.menu-main-menu-container').addClass('mobile');}});setTimeout(function(){$('.slick-active .cover_block .desctop').addClass('active');},300);setTimeout(function(){$('.slick-active .cover_block .notebook').addClass('active');},400);setTimeout(function(){$('.slick-active .cover_block .planshet').addClass('active');},500);setTimeout(function(){$('.slick-active .cover_block .mobile').addClass('active');},600);$('.portfolio-slider').on('afterChange',function(slick,currentSlide){$('.cover_block .desctop,.cover_block .notebook,.cover_block .planshet,.cover_block .mobile').removeClass('active');setTimeout(function(){$('.slick-active .cover_block .desctop').addClass('active');},0);setTimeout(function(){$('.slick-active .cover_block .notebook').addClass('active');},100);setTimeout(function(){$('.slick-active .cover_block .planshet').addClass('active');},200);setTimeout(function(){$('.slick-active .cover_block .mobile').addClass('active');},300);});});
